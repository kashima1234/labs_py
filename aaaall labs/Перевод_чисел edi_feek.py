...
def main():
    ...
    root.title("Calculator 2rd base first lab")

    # to get number from keyb from userseseser
    def get_text(x):
        if entry_box.get() == 'O':
    ...
    def clear_hist():
        result_list.clear()
        status_bar.configure(text ="History: " + "|".join(result_list)) #  c his 
    ...
    def about():
        messagebox.showinfo("About", "2rd Base Calculator by Me\n@All right's reserved by me ;))!")

    def help():
        messagebox.showinfo("Help", "1. To do mathematical operation just type and do it.\n2.for change from base 10 to base 2 click to change the number.\
            \n3.To change from 10 base to 2 just add the number and click The button or use the menu option for doing operation.\n@All right's reserved!")
    ...
    def decimal_second():
        A = entry_box.get()
        if not isit_num(A):
            messagebox.showerror("Error", "Entered symbols not number.")
            entry_box.delete(0, END)
        else:
            if A[0] == '-':
                sign = -1
    ...
    def second_decimal():
        ...
        if not isit_num(A):
            ...
            entry_box.delete(0, END)
        elif ('2' in A) or ('3' in A) or ('4' in A) or ('5' in A) or ('6' in A) or ('7' in A) or ('8' in A) or ('9' in A):
            messagebox.showerror("Error", "in Ternary only 0, 1, 2, 3")
            entry_box.delete(0, END)
        else:
            if A[0] == '-':
                sign = -1
            ...
            for i in range(len(chars[0])-1, -1, -1):
                m += 1
                result += chars[0][i]*(2**m)
            m = 0
            for i in range(len(chars[1])):
                m -= 1
                result += chars[1][i]*(2**m)
    ...
    filemenu.add_command(label="Second Base", command=decimal_second)
    filemenu.add_command(label="Decimal", command=second_decimal)
    ...
    btn_third = ttk.Button(text = "Second Base", command = decimal_second,style='e.TButton')
    ...
    btn_clear_history.place(x = 160,y = 300,width=70, height = 50)
   # btn_clear_history.place(x = 160,y = 300)
    btn_decimal = ttk.Button(text = "Decimal", command = second_decimal,style='e.TButton')
