...
#Lab 10     {Трапеций и тревосмых метод}
from math import *
...
def input_control():
    ...
    try:
        # while s1 <= 0:
        #     s1 = float(input(f"Split step 1 for numerical integration: "))
        # while s2 <= 0 or s2 == s1:
        #     s2 = float(input(f"Split step 2 for numerical integration: "))
        
        start = float(input(f"Initial value: "))
        end = start
        while end <= start:
            end = float(input(f"End value: "))
        # n1 = int((end - start) / s1)
        # n2 = int((end - start) / s2)
        n1 = n2 = 0
...
print('|{:^10}|{:^20.4f}|{:^20.4f}|'.format('Method 1', method1[0], method1[1]))
print('|{:^10}|{:^20}|{:^20}|\n'.format('Method 2', round(method2[0], 4) if n1 % 3 == 0 else 'not allowed!',
                                        round(method2[1], 4) if n2 % 3 == 0 else 'not allowed!'))
...
print(
    '|{:^40}|{:^20.4f}|{:^20.4f}|'.format('Absolute error Method 1', error_method1_n1[0],
                                          error_method1_n2[0]))
print('|{:^40}|{:^20.3%}|{:^20.3%}|'.format('Relative error Method 1', error_method1_n1[1],
                                            error_method1_n2[1]))
print(
    '|{:^40}|{:^20}|{:^20}|'.format('Absolute error Method 2',
                                    str(round(error_method2_n1[0], 4)) if n1 % 3 == 0 else 'not allowed!',
                                    str(round(error_method2_n2[0], 4)) if n2 % 3 == 0 else 'not allowed!'))
print('|{:^40}|{:^20}|{:^20}|'.format('Relative error Method 2',
                                      str(round(error_method2_n1[1], 4) * 100) + '%' if n1 % 3 == 0 else 'not allowed!',
                                      str(round(error_method2_n2[1], 4) * 100) + '%' if n2 % 3 == 0 else 'not allowed!'))

if abs(error_method1_n1[0]) >= abs(error_method2_n1[0]):
    act = method_трапеций
    print('Less accurate method: Трапеций метод')
else:
    act = method_тревосмых
    print('Less accurate method: Тревосмых метод')
eps = 0
while eps <= 0:
    try:
        eps = float(input('\nAccuracy='))
