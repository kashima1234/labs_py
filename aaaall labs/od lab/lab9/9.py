...
print('Third-demensional D array input')
print('To exit press ENTER on the first line of the new matrix')
slice_idx = 0
while True:
    print()
    print('Input ', slice_idx+1, 'layer', sep='')
    print('Enter matrix values line by line, separated by spaces!')
    str_idx = 0
    while True:
        ...
        if str_idx > 0 or slice_idx > 0:
            if len(values) > len(A[0][0]):
                print('The entered number of elements cannot exceed the number of \
elements of the first row of the first matrix')
                continue
            for i in range(len(values), len(A[0][0])):
                values += [None]
        if str_idx == 0:
            A += [[]]
        A[slice_idx] += [values]
