...
def align(a,b):   #to make a and b equal in length and to align the '.'
    ...
    if p_ind1 == -1 and p_ind2 > -1:
        a = a + '.' + '0' * len2   # if first number is integer i am making it float
    ...
    if len(a)>len(b):
        b = '0'*prefx + b  #adding 0 before and integer
    else:
        a =  '0'*prefx + a
...
def Subtract(a,b):  # gives a-b
    ...
    for i in reversed(range(len(a))):
        d1 = a[i] #revers first number 
        d2 = b[i]  #reverse second number
        if d1 >= d2:
