x = float(input('Введите начало интервала: '))
b = float(input('Введите конец интервала: '))      #Выводим начало, конец и шаг интервала
c = float(input('Введите шаг: '))
print(' -----------------------------')
print('|    x     |       f(x)       |')
ymax = 512*x**10-1280*x**8-1120*x**6-400*x**4-5*x**2-1
ymin = 512*x**10-1280*x**8-1120*x**6-400*x**4-5*x**2-1
i=x
while i <= b:
    y = 512*i**10-1280*i**8-1120*i**6-400*i**4-5*i**2-1
    if y > ymax:
        ymax = y
...
while i <= b:
    ...
    f = 512*i**10-1280*i**8-1120*i**6-400*i**4-5*i**2-1    #Выводим значения для ОХ
    ...
    if ymin*ymax > 0:
        ...
        if p0 == pr:
            if f == 0:                                    #Рисуем график
                print(" "*(p0)+"+")
                print('Эту функцию сечет график в '.format(po))
            elif f > 0:
                print(" "*(p0)+"|*")
            else:
                print(" " * (p0 - 1) + "*|")
